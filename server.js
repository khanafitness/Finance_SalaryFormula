...
// ── Health check (used by edge function to wake Render) ──
...
async function launchBrowser() {
  return puppeteer.launch({
    executablePath:
      ...
      "/opt/render/project/.chrome/chrome/linux-stable/chrome-linux64/chrome",
    args: [
      "--no-sandbox",
...
async function renderToPdf(browser, templatePath, data) {
  ...
  try {
    await page.setContent(html, {
      waitUntil: "domcontentloaded", // no external network calls → safe
      timeout: 30000,
    });
...
// Body: { customer_name, email, mobile_number }
// Returns: PDF binary
